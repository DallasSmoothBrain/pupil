...
## TODO each of these needs to return a channel or a list of channels. Each channel has an associated querry function

# def gaze_on_surf():
#     return [
#         Channel(
#             query=partial(gaze_on_surf_q, index=i),
#             label="gaze_on_surf_" + 'xy'[i],
#             eye='both',
#             metatype='surface_coords',
#             unit='pix',
...
def img_to_surf_trans():
    ...
    for i in range(3):
        for j in range(3):
            ls.append(
                Channel(
            query=partial(img_to_surf_trans_q, index_a=i, index_b=j),
            label=f'image2surface_transform_{i}_{j}',
            eye='both',
            metatype='image2surface_transform',
        )
            )
    
    return ls

def surface_name_q(surface_datum):
    return surface_datum['name']

def img_to_surf_trans_q(surface_datum, index_a, index_b):
    return surface_datum['img_to_surf_trans'][index_a][index_b]

# def gaze_on_surf_q(surface_datum, index):
#     return surface_datum['gaze_on_surf'][index]
